...
class LoanWorkflow(db.Model):
    ...
    # Post-approval controls: FE approval -> DO approval -> FE TVR -> DO disbursement.
    fe_approved_at = db.Column(db.DateTime)
    fe_approval_remark = db.Column(db.Text)
    tvr_submitted_at = db.Column(db.DateTime)
    tvr_remark = db.Column(db.Text)
    disbursed_at = db.Column(db.DateTime)
    disbursed_by = db.Column(db.Integer, db.ForeignKey("user.id"))
    disbursement_remark = db.Column(db.Text)
...
class Dealer(db.Model):
    ...
    portal_modules = db.Column(db.Text, default="")                # Dealer Portal module rights, comma-separated
