...
export function CashAtDealerPage(){
  ...
  return <div>
    <div className="dealerContentToolbar">
      <div className="dealerPageIntro"><span className="dealerSectionIcon">₹</span><div><strong>Cash at Dealer</strong><small>Cash received − dealer expenses − cash handed over to Head Office</small></div></div>
    ...
    <div className="tablewrap dealerTable"><table className="table"><thead><tr><th>Dealer</th><th>Cash Received</th><th>Expenses</th><th>HO Handover</th><th>Cash at Dealer</th></tr></thead><tbody>{(data?.rows||[]).map(r=><tr key={r.dealer_id}><td><b>{r.dealer_code||''}</b> {r.dealer_name}</td><td><Money value={r.cash_received}/></td><td><Money value={r.expenses}/></td><td><Money value={r.ho_handover}/></td><td><b><Money value={r.cash_at_dealer}/></b></td></tr>)}{!loading&&!data?.rows?.length&&<tr><td colSpan="5" className="dealerEmpty">No dealer cash records found.</td></tr>}</tbody></table></div>
