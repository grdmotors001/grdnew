...
const emptyEmployee = { employee_code:'', name:'', department:'HR', designation:'', mobile:'', joining_date:'', machine_user_id:'', basic_salary:'', hra:'', other_allowance:'', overtime_rate:'' };
    ...
    {tab==='employees' && <>
      <form className="card" onSubmit={saveEmployee}>
        ...
        <div className="formgrid">
          {[['employee_code','Employee Code',true],['name','Name',true],['department','Department',false],['designation','Designation',false],['mobile','Mobile',false],['joining_date','Joining Date',false,'date'],['machine_user_id','Punch Machine User ID',false],['basic_salary','Basic Salary',false,'number'],['hra','HRA',false,'number'],['other_allowance','Other Allowance',false,'number'],['overtime_rate','Overtime Rate / Hr',false,'number']].map(([k,l,req,type])=><div className="field" key={k}><label>{l}</label><input type={type||'text'} value={form[k]} required={req} onChange={e=>setForm({...form,[k]:e.target.value})}/></div>)}
        </div>
        <div className="actions" style={{marginTop:16}}><button className="btn primary" disabled={busy}>{busy?'Saving…':'Add Employee'}</button></div>
      </form>
      <div className="card tableWrap" style={{marginTop:16}}><table className="reportTable"><thead><tr><th>Code</th><th>Name</th><th>Department</th><th>Designation</th><th>Machine ID</th><th>Basic</th><th>Status</th></tr></thead><tbody>
        {employees.map(e=><tr key={e.id}><td>{e.employee_code}</td><td>{e.name}</td><td>{e.department}</td><td>{e.designation||'-'}</td><td>{e.machine_user_id||'-'}</td><td>{Number(e.basic_salary||0).toFixed(2)}</td><td>{e.active?'Active':'Inactive'}</td></tr>)}
        {!employees.length&&<tr><td colSpan="7" className="muted">No employees found.</td></tr>}
      </tbody></table></div>
    </>}
