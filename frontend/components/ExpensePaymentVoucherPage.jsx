...
export function ExpensePaymentVoucherPage(){
  ...
  const [rickshaws,setRickshaws]=useState([]),[rows,setRows]=useState([]);
  ...
  return <div className="page">
    ...
    {loading?<div className="muted">Loading…</div>:<div className="tablewrap"><table className="table"><thead><tr><th>Date</th><th>Voucher</th><th>Pay To</th><th>Expense</th><th>Mode</th><th>Amount</th><th>Status</th><th>Bill</th><th>Action</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{r.date}</td><td><b>{r.voucher_no}</b></td><td>{r.pay_to_name}</td><td>{r.expense_type_name}{r.chassis_no?' — '+r.chassis_no:''}</td><td>{r.payment_mode}</td><td>{money(r.amount)}</td><td><b>{r.status}</b>{r.rejection_reason?' — '+r.rejection_reason:''}</td><td>{r.bill_no||'—'}</td><td>{r.status==='pending'?<><button className="btn" onClick={()=>approval(r.id,'approve')} style={{marginRight:6}}>Approve</button><button className="btn" onClick={()=>approval(r.id,'reject')}>Reject</button></>:'—'}</td></tr>)}{!rows.length&&<tr><td colSpan="9" className="muted">No expense vouchers found.</td></tr>}</tbody></table></div>}</div>
  </div>
}