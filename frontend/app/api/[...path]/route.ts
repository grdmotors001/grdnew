  ...
  try{
    ...
    if(p==="dashboard"){
      const [vehicles,stages,monthly,billed,states,dealers,pending,sales,production]=await Promise.all([
        ...
        pool.query("SELECT TO_CHAR(date,'YYYY-MM') AS month,COUNT(*) FILTER (WHERE stage='Manufacturing')::int AS manufacturing,COUNT(*) FILTER (WHERE stage='Delivery Challan')::int AS delivery_challan,COUNT(*) FILTER (WHERE stage='Tax Invoice')::int AS tax_invoice FROM vehicle WHERE date >= date_trunc('month',CURRENT_DATE)-INTERVAL '11 months' GROUP BY 1 ORDER BY 1"),
        pool.query("SELECT TO_CHAR(date,'YYYY-MM') AS month,COUNT(*)::int AS billed,COALESCE(SUM(GREATEST(COALESCE(gst_sale_amount,sale_amount,0)-COALESCE(discount,0),0)),0)::numeric AS taxable FROM tax_invoice WHERE COALESCE(cancelled,false)=false AND date >= date_trunc('month',CURRENT_DATE)-INTERVAL '11 months' GROUP BY 1 ORDER BY 1"),
        pool.query("SELECT COALESCE(NULLIF(TRIM(buyer_state),''),'Unknown') AS state,COUNT(*)::int AS billed,COALESCE(SUM(GREATEST(COALESCE(gst_sale_amount,sale_amount,0)-COALESCE(discount,0),0)),0)::numeric AS taxable FROM tax_invoice WHERE COALESCE(cancelled,false)=false AND date >= date_trunc('month',CURRENT_DATE)-INTERVAL '11 months' GROUP BY 1 ORDER BY taxable DESC,state"),
